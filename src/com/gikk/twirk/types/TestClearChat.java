...
import com.gikk.twirk.types.GikkDefault_TwitchMessageBuilder;
...
public class TestClearChat {
	...
	private static void testMessage(String MESSAGE, CLEARCHAT_MODE mode, String target, int duration, String reason) {
		...
		org.junit.Assert.assertTrue("Got: " + clearChat.getMode() + " Expected: " + mode, clearChat.getMode() == mode);
		org.junit.Assert.assertTrue("Got: " + clearChat.getDuration() + " Expected: " + duration, clearChat.getDuration() == duration);
		org.junit.Assert.assertTrue("Got: " + clearChat.getTarget()+ " Expected: " + target, clearChat.getTarget().equals(target) );
		org.junit.Assert.assertTrue("Got: " + clearChat.getReason()+ " Expected: " + reason, clearChat.getReason().equals(reason) );
		String raw = clearChat.getRaw();
		org.junit.Assert.assertTrue("Got: " + clearChat.getRaw()+ " Expected: " + MESSAGE , MESSAGE.equals(raw) );
	}
}
