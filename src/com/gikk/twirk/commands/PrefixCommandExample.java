...
public class PrefixCommandExample extends CommandExampleBase {
	...
	protected void performCommand(String command, TwitchUser sender, TwitchMessage message) {
		if( command.equals(patternA) )
			twirk.channelMessage(sender.getUserName() +": Local time zone is " + Calendar.getInstance().getTimeZone().getDisplayName());
		else if( command.equals(patternB) )
			twirk.channelMessage(sender.getUserName() +": Local time is " + new Date().toString() );
